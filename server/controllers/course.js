...
export const addLesson = async (req, res) => {
  ...
  const newLesson = await Lesson.create({...req.body, chapter : chapterId, slug: slugify(title)})
  await Chapter.findByIdAndUpdate(chapterId, { $push : { lessons : newLesson }})
  const foundChapter = await Chapter.findById(chapterId).populate('lessons');
  console.log("leçon ajoutée : "+ newLesson + " au chapitre " +foundChapter);
  res.json({
      status : 'Succès',
